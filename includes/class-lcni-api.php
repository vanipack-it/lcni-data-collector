...
class LCNI_API {
    ...
    private static function is_symbol_supported_for_entrade($symbol) {
        ...
        if (!is_array($symbol_map) || empty($symbol_map)) {
            self::$last_request_error = 'Unable to validate supported symbols for Entrade chart-api. Falling back to symbol format validation.';
            LCNI_DB::log_change('api_warning', self::$last_request_error, ['symbol' => $symbol]);
