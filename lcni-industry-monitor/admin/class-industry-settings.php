...
class LCNI_Industry_Settings
{
    ...
    public function sanitize_settings($input)
    {
        ...
        return array(
            ...
            'dropdown_border_color' => sanitize_hex_color($input['dropdown_border_color'] ?? $defaults['dropdown_border_color']) ?: $defaults['dropdown_border_color'],
     ...
     * @return array<int,array{field:string,start_color:string,mid_color:string,end_color:string}>
     */
    private function sanitize_row_gradient_rules($rules, $allowed_fields)
    {
        ...
        foreach ($rules as $rule) {
            ...
            $start_color = sanitize_hex_color($rule['start_color'] ?? '#ffffff') ?: '#ffffff';
            $mid_color = sanitize_hex_color($rule['mid_color'] ?? '#f3f4f6') ?: '#f3f4f6';
            $end_color = sanitize_hex_color($rule['end_color'] ?? '#dcfce7') ?: '#dcfce7';
        ...
        <div class="wrap">
            ...
            <form method="post" action="options.php">
                ...
                <table class="form-table" role="presentation">
                    ...
                    <tr><th><label><?php echo esc_html__('Dropdown width (px)', 'lcni-industry-monitor'); ?></label></th><td><input type="number" min="160" max="520" name="<?php echo esc_attr(self::OPTION_KEY); ?>[dropdown_width]" value="<?php echo esc_attr((string) $settings['dropdown_width']); ?>" /></td></tr>
                    <tr><th><label><?php echo esc_html__('Dropdown border color', 'lcni-industry-monitor'); ?></label></th><td><input type="color" name="<?php echo esc_attr(self::OPTION_KEY); ?>[dropdown_border_color]" value="<?php echo esc_attr($settings['dropdown_border_color']); ?>" /></td></tr>
                ...
                <h2><?php echo esc_html__('Row gradient rules', 'lcni-industry-monitor'); ?></h2>
                <p><?php echo esc_html__('Apply row-level gradient colors based on each row minimum and maximum value.', 'lcni-industry-monitor'); ?></p>
                <table class="widefat" id="lcni-row-gradient-rules-table" style="max-width:1000px;">
                    <thead>
                        <tr>
                            <th><?php echo esc_html__('Field', 'lcni-industry-monitor'); ?></th>
                            <th><?php echo esc_html__('Start background', 'lcni-industry-monitor'); ?></th>
                            <th><?php echo esc_html__('Middle background', 'lcni-industry-monitor'); ?></th>
                            <th><?php echo esc_html__('End background', 'lcni-industry-monitor'); ?></th>
                            <th></th>
                        </tr>
                    </thead>
                    <tbody id="lcni-row-gradient-rules-body">
                        <?php foreach ((array) ($settings['row_gradient_rules'] ?? array()) as $index => $rule) : ?>
                            <?php $this->render_row_gradient_rule_row((int) $index, (array) $rule, $metric_labels); ?>
                        <?php endforeach; ?>
                    </tbody>
                </table>
                <p><button type="button" class="button" id="lcni-add-row-gradient-rule"><?php echo esc_html__('+ Add row gradient rule', 'lcni-industry-monitor'); ?></button></p>

                <template id="lcni-row-gradient-rule-template">
                    <?php $this->render_row_gradient_rule_row('__INDEX__', array('field' => '', 'start_color' => '#ffffff', 'mid_color' => '#f3f4f6', 'end_color' => '#dcfce7'), $metric_labels); ?>
                </template>
        ...
        <script>
            (function () {
                ...
                function bindRemoveActions(scope) {
                    ...
                    scope.querySelectorAll('.lcni-remove-rule').forEach(function (btn) {
     ...
     * @param array<string,string> $metric_labels
     */
    private function render_row_gradient_rule_row($index, $rule, $metric_labels)
    {
        ?>
        <tr>
            <td>
                <select name="<?php echo esc_attr(self::OPTION_KEY); ?>[row_gradient_rules][<?php echo esc_attr((string) $index); ?>][field]">
                    <option value=""><?php echo esc_html__('Select field', 'lcni-industry-monitor'); ?></option>
                    <?php foreach ($metric_labels as $metric_key => $label) : ?>
                        <option value="<?php echo esc_attr($metric_key); ?>" <?php selected((string) ($rule['field'] ?? ''), $metric_key); ?>><?php echo esc_html($label); ?></option>
                    <?php endforeach; ?>
            ...
            <td><input type="color" name="<?php echo esc_attr(self::OPTION_KEY); ?>[row_gradient_rules][<?php echo esc_attr((string) $index); ?>][start_color]" value="<?php echo esc_attr((string) ($rule['start_color'] ?? '#ffffff')); ?>" /></td>
            <td><input type="color" name="<?php echo esc_attr(self::OPTION_KEY); ?>[row_gradient_rules][<?php echo esc_attr((string) $index); ?>][mid_color]" value="<?php echo esc_attr((string) ($rule['mid_color'] ?? '#f3f4f6')); ?>" /></td>
            <td><input type="color" name="<?php echo esc_attr(self::OPTION_KEY); ?>[row_gradient_rules][<?php echo esc_attr((string) $index); ?>][end_color]" value="<?php echo esc_attr((string) ($rule['end_color'] ?? '#dcfce7')); ?>" /></td>
            <td><button type="button" class="button-link-delete lcni-remove-rule">×</button></td>
        </tr>
        <?php
    }
