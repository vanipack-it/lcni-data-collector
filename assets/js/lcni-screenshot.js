  ...
  <div class="lcni-ss-editor-bottombar">
    ...
    <button class="lcni-ss-action-btn primary" id="lcni-ed-done">✅ Hoàn tất & Lưu</button>
        ...
        async function buildFramedCanvas(srcCanvas) {
            ...
            // Ngày giờ xuất ảnh — góc phải bar dưới (thay logo watermark opacity)
            {
                const now   = new Date();
                ...
                const dtStr = `${dd}/${mo}/${yyyy}  ${hh}:${min}`;
                const dtSize = Math.max(10, Math.round(fp.textSize * sc * 1.2));
                ctx.save();
                ctx.font        = `600 ${dtSize}px "DM Sans",system-ui,sans-serif`;
                ctx.fillStyle   = fp.textClr;
                ctx.globalAlpha = 0.75;
                ctx.shadowBlur  = 4;
                ctx.shadowColor = 'rgba(0,0,0,0.45)';
                const dtW = ctx.measureText(dtStr).width;
                const dtX = SM + W - PAD - dtW;
                const dtY = barY + Math.round(barH / 2) + Math.round(dtSize * 0.36);
                ctx.fillText(dtStr, dtX, dtY);
                ctx.restore();
            }
        ...
        async function getExportCanvas() {
            ...
            if (wmApplied && !frameApplied) {
                // Burn watermark vào clone — chỉ khi KHÔNG dùng frame
                // (frame đã có logo + ngày giờ trong bar dưới, không burn logo watermark thêm)
        ...
        modal.querySelector('#lcni-ed-done').addEventListener('click', async () => {
            ...
            // Nếu đã dùng frame → bar dưới đã có logo + ngày giờ, không burn watermark thêm
            if (wmApplied && !frameApplied) {
                await burnWatermark();
            } else if (!frameApplied) {
