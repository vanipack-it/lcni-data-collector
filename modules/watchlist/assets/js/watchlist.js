(function () {
  ...
  function renderRowsMarkup(orderedColumns, items, stickyColumn, valueColorRules, cellToCellRules) {
  ...
  function resolveCellToCellMeta(column, row, rules) {
  ...
  function bindGlobalDelegation() {
    ...
    document.addEventListener('click', async (event) => {
      ...
      if (sortTh) {
        ...
        // Chỉ update tbody rows + sort icons — KHÔNG rebuild toàn bộ DOM
        // Giữ nguyên scroll position và sticky state
        const tbody = host.querySelector('tbody');
        if (tbody) {
          ...
          const orderedColumns = Array.from(host.querySelectorAll('thead th[data-sort-key]'))
            .map(th => String(th.getAttribute('data-sort-key') || '').trim()).filter(Boolean);
          tbody.innerHTML = renderRowsMarkup(orderedColumns, dataset, stickyColumn, valueColorRules, cellToCellRules);
        }

        // Cập nhật sort icon trong thead — không động đến scroll container
        host.querySelectorAll('thead th[data-sort-key] .lcni-sort-icon').forEach(span => {
          const thKey = span.closest('th').getAttribute('data-sort-key');
          span.textContent = thKey === watchlistSortKey ? (watchlistSortDir === 'asc' ? '↑' : '↓') : '';
        });
