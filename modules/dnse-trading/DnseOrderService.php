...
class LCNI_DnseOrderService {
    ...
    /**
     * Lấy sức mua của tiểu khoản từ DNSE API.
     *
     * Trả về:
     *   - buying_power   : số tiền mua tối đa (VNĐ)
     *   - max_volume     : khối lượng mua tối đa theo giá (làm tròn xuống 100 cổ)
     *   - account_balance: số dư tài khoản hiển thị
     *   - symbol         : mã CK (nếu truyền vào)
     *   - price          : giá dùng để tính (DB format — nghìn VNĐ)
     *
     * @param int    $user_id
     * @param string $account_no   Số tài khoản DNSE
     * @param string $account_type 'spot' | 'margin'
     * @param string $symbol       Mã CK (tuỳ chọn — để API trả chính xác hơn)
     * @param float  $price        Giá DB format (21.5 = 21,500 VNĐ). 0 = bỏ qua
     *
     * @return array|WP_Error
     */
    public function get_buying_power(
        int    $user_id,
        string $account_no,
        ...
        $raw = $this->api->get_trade_capacities( $jwt, $account_no, $account_type, $symbol, $price );
        if ( is_wp_error( $raw ) ) return $raw;

        // DNSE trade-capacities response fields (theo API docs DNSE):
        // purchasingPower / buyingPower / pp — sức mua tiền mặt (VNĐ)
        // maxBuyQuantity / maxVolume         — khối lượng mua tối đa
        // accountBalance / totalAsset        — số dư tổng
        $buying_power = (float) (
            $raw['purchasingPower'] ??
            $raw['buyingPower']     ??
            $raw['pp']              ??
        ...
        // Nếu API trả trực tiếp max qty
        $api_max_vol = isset( $raw['maxBuyQuantity'] ) ? (int) $raw['maxBuyQuantity']
                     : ( isset( $raw['maxVolume'] )   ? (int) $raw['maxVolume'] : 0 );

        // Tự tính max qty nếu API không trả, hoặc làm fallback
        $price_vnd = $price * 1000; // DB format → full VNĐ
        $calc_max_vol = ( $price_vnd > 0 && $buying_power > 0 )
            ? (int) ( floor( $buying_power / $price_vnd / 100 ) * 100 )
            : 0;

        $max_volume = $api_max_vol > 0 ? $api_max_vol : $calc_max_vol;
